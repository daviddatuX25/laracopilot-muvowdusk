<div wire:poll-5000ms="loadNotifications" class="ml-3 relative" x-data="{ notificationOpen: false }">
    <button @click="notificationOpen = !notificationOpen" class="relative inline-flex items-center text-sm font-medium text-center text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 focus:outline-none" type="button">
    ...
    <div x-show="notificationOpen"
         ...
         class="absolute right-0 mt-2 w-80 md:w-96 max-w-sm md:max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-gray-700 z-50"
         style="display: none;">
        ...
        <div class="px-3 md:px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            ...
            @if($unseenCount > 0)
                <span class="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200">
                    {{ $unseenCount }}
                </span>
            @endif
        </div>
                ...
                @foreach($notifications as $notification)
                    <div wire:key="notification-{{ $notification->id }}"
                         @class([
                             ...
                             'ring-l-4 ring-blue-500 bg-blue-50' => !$notification['is_seen'],
                             'bg-red-50' => $notification['is_seen'] && $notification['type'] === 'out_of_stock',
                             'bg-yellow-50' => $notification['is_seen'] && $notification['type'] !== 'out_of_stock',
                         ])
