...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-white flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow-2xl sm:rounded-lg sm:px-10 border border-slate-200/50">
                @if($errors->any())
                ...
                <div class="mb-6 bg-blue-50 border-l-4 border-blue-500 p-4">
                    <h3 class="text-lg font-medium text-blue-800 mb-2">Admin Credentials:</h3>
                    <ul class="list-disc pl-5 space-y-1 text-sm text-blue-700">
                        @foreach($credentials as $email => $data)
                            <li>Email: <strong>{{ $email }}</strong> | Password: <strong>{{ $data['password'] }}</strong></li>
                        @endforeach
                    </ul>
                </div>
